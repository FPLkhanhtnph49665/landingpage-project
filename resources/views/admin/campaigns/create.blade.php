...
@section('pageHeader', 'Tạo chiến dịch')
@section('pageSubheader', 'Thêm chiến dịch thiện nguyện mới')

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="mb-1">Tạo chiến dịch</h3>
            <p class="text-muted mb-0">Thêm chiến dịch mới vào hệ thống.</p>
        </div>
            ...
            <form action="{{ route('admin.campaigns.store') }}" method="POST">
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="number" name="target_amount" value="{{ old('target_amount') }}" class="form-control @error('target_amount') is-invalid @enderror" min="0">
                        @error('target_amount')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Ngày bắt đầu</label>
                        <input type="date" name="start_at" value="{{ old('start_at') }}" class="form-control @error('start_at') is-invalid @enderror">
                        @error('start_at')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Ngày kết thúc</label>
                        <input type="date" name="end_at" value="{{ old('end_at') }}" class="form-control @error('end_at') is-invalid @enderror">
                ...
                <button type="submit" class="btn btn-success">Lưu chiến dịch</button>
            </form>
        </div>
    </div>
@endsection
